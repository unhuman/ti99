...
figures "did not get there". One step is a sixteenth and a bar is 16 steps.

THE TEMPO IS NOT SET HERE. At the MIDI's own 151 BPM a step is 60/151/4 =
0.0993 s = 6 frames, and that is what the game played for a long time. It
now runs MUSTICK 5 -- 180 BPM -- by choice. This file only decides what the
notes ARE; RALLYX.bas decides how fast they go by.
...
# Kept in step with RALLYX.bas's CONST MUSTICK -- this only reports the loop
# length, it does not set the tempo.
MUSTICK = 5
print("wrote src/music.bas: %d steps = %d bars = %.1f s at %d frames/step (%d BPM)"
      % (STEPS, STEPS // 16, STEPS * MUSTICK / 60.0, MUSTICK, round(900.0 / MUSTICK)))
