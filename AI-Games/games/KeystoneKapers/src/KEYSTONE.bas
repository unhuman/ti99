...
setup_rest:
	...
	' The four lamps as shipped (CH_BULB0..3). The NES carries only these 32
	' bytes of store_pat -- genart.py emits them as lamp_nes_pat.
	#nsrc = VARPTR lamp_nes_pat(0)
	nchr = CH_BULB0
	ncnt = 4
	ntab = 1
	#ncol = 0
...
esc_tick:
	...
	IF klsc = 0 THEN
		...
		' THE SAME COLOURS THE SETUP LOAD GAVE THESE CHARACTERS: store_col's
		' bytes for the escalator run, which is all of store_col the NES
		' carries -- genart.py emits it as nes_esc_col, sliced from the codes
		' (ESC_FIRST), so a renumber moves it with them. 110 is its first.
		#ncol = VARPTR nes_esc_col(0)
		GOSUB nes_escd
		#else
	...
	ELSE
		...
		#ncol = VARPTR nes_esc_col(0)
		#ncol = #ncol + 48		' character 116, six on from 110
		GOSUB nes_escd
		#else
...
setup_font:
	...
	ncnt = 59
	ntab = 1
	' THE FONT'S COLOUR, $B4 on every row of all 59 characters: light yellow
	' on dark blue, ink index 3 on paper index 1, and which colours those are
	' is then the attribute table's business, per region. It used to be a
	' 472-byte table of that one value (nes_fcol), kept because "ROM is not
	' the scarce budget" -- on the NES it is. A #ncol under $100 is the
	' colour byte itself (assets/nes_chr.asm, nes_chrinkrow).
	#ncol = $B4
	nink = 3
	GOSUB nes_def
	#else
