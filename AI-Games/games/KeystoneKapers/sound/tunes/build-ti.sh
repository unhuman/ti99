...
SRC="TUNES.bas"
NAME="TUNES"
CARTNAME="KEYSTONE TUNES"   # the TI menu entry; not a filename, so it
...
# GENERATE. src/songs.bas is written by assets/gentunes.py from the bar lists.
rm -rf ../assets/__pycache__
"$PY" ../assets/gentunes.py || die "gentunes.py failed"
...
# everybody ignores.
# NO GENERATE STAGE AND NO GATES. Keystone Kapers' build regenerates its art and
# then runs thirteen checks over it; this program has no art, no levels and no
# actors, so all of that came out. What is kept is the part that is about the
# TOOLCHAIN rather than about that game -- the PATH order, the cygpy argument
# handling, the 8-bit truncation sweep -- because those bite any CVBasic build
# on this machine.
