...
# THE NES PLAYS THE FIRST HALF OF THE TITLE TUNE. With both 16-bar tunes its
# PRG came out 56 bytes over (Keystone DESIGN.md section 50). The in-game tune
# is the one heard for minutes at a time, so it stays whole and the title gives
# way: eight bars of STREET free about 258 bytes, and its first half ends on its
# C7 and turns cleanly back to the top. The TI and ColecoVision keep both full
# tunes, under #else, byte for byte as before.
NES_TITLE_BARS = 8
