...
#
# HE HAS TO KEEP HIS MASS. The first crouch narrowed to five pixels through the
# middle, so ducking did not read as a man bending -- it read as the figure
# shrinking to a smudge and then coming back. A crouch does not lose volume, it
# REDISTRIBUTES it: the back rounds up and the haunches thicken as the height
# comes down. So the widest rows here are wider than his standing torso, not
# narrower, and nothing tapers below the shoulders except the legs.
#
# The body is the WHOLE silhouette, head to heel, and the hat and face are
# drawn OVER it from lower slots -- so widening it can only ever add visible
# bulk at the edges, never disturb the two features that say "Kelly".
#
...
# faces the other way.
#
# The three grids that used to sit here are assets/kelly-duck.txt
# now -- see _kelly_duck() below. This commentary stays because it
# is about the DESIGN of the pose, which the file's own header does
# not repeat.
...
# THE CROUCH IS A FILE NOW TOO (assets/kelly-duck.txt), for the same reason the
# run poses are: it was three triple-quoted strings up at the top of this file,
# which meant "edit the crouch" was a Python edit and the drawing lived where
# nobody would look for it.
KELLY_DTOP, KELLY_DLEG = _kelly_pose("kelly-duck.txt")
...
# HE IS STRIPED FROM SHOULDER TO HEM, which is what the reference shows: a
# stack of alternating light and dark bands, and the ARMS ARE THOSE SAME BANDS
# EXTENDED SIDEWAYS. That is what makes them read as arms rather than as lumps
# beside the body -- see the note over HARRY_TOP.
#
# Bands on 10, 12 and 14 leave white on 9, 11, 13 and 15, so the torso is seven
# rows of alternating stripe. It used to be {3, 13, 15}: two bands down near the
# waist and nothing above them, which read as a belt rather than as a shirt.
#
# Row 3 is the CAP's black band and is not part of the shirt.
HSTRIPE = {0, 2, 10, 12, 14}
HCAP = set(range(0, 3))       # the hat is THREE rows now, and has no brim
HFACEB = set(range(3, 9))       # was 4-10: his head was 44% of the figure;
                                # starts at 3 now that the brim row is gone
HBODYB = set(range(9, 16))      # shoulders down, so the arms are IN the body
...
def split_stripes(top, arms, bands):
    """Split a figure into its WHITE half and its BLACK half.

    A striped figure is drawn as TWO sprites whose pixels never overlap -- the
    stripe shows through holes in the body, it is not painted over it. That is
    forced by the hardware: the lower-numbered sprite wins every shared pixel,
    so a black sprite BEHIND a solid white one is simply invisible. (Harry's
    trousers stayed white through two builds for exactly this reason: the leg
    stripes were a whole sprite drawn under a solid leg.)

    Ordinary cells take the colour of their row -- black on a band row, white
    between. WHERE AN ARM CROSSES THE BODY THE TWO INVERT, so the arm reads as
    a limb passing in front of him rather than vanishing into the torso: a
    white row shows a black sleeve, a black band shows a white one.
    """
    trows = top.strip(chr(10)).split(chr(10))
    arows = arms.strip(chr(10)).split(chr(10))
    white, black = [], []
    for i, (tr, ar) in enumerate(zip(trows, arows)):
        wr, br = [], []
        for j, tc in enumerate(tr):
            # THE FACE HAS ITS OWN SPRITE, so its rows must be blank in both of
            # these or the skin band and the white band fight for the same
            # pixels. band() used to do this implicitly by only keeping
            # HCAP | HBODYB; splitting cell by cell has to say it out loud.
            if tc != "#" or i in HFACEB:
                wr.append(".")
                br.append(".")
                continue
            dark = i in bands
            # a cell that is BOTH arm and body is a crossing -- invert it
            if ar[j] == "#" and TORSO_MASK[i][j] == "#":
                dark = not dark
            wr.append("." if dark else "#")
            br.append("#" if dark else ".")
        white.append("".join(wr))
        black.append("".join(br))
    nl = chr(10)
    return (nl + nl.join(white) + nl, nl + nl.join(black) + nl)


# ==========================================================================
# HARRY'S RUN COMES FROM 1.txt .. 4.txt -- four hand-drawn 16x24 poses.
#
# OUR HEAD, THEIR BODY. The drawn frames carry their own head, and it is not
# the one the rest of the game was built around -- the cap sits differently and
# the face band would land on the wrong rows, which would break HFACEB and the
# separate face sprite with it. So rows 0-7 are taken from the HARRY_TOP art
# above (unchanged, still the head we drew) and rows 8-23 from the new frame.
# The seam is row 8, which is the neck in both, so nothing has to be redrawn to
# make them meet.
#
# WHAT EACH FRAME PROVIDES:
#   rows  0-7   head      -- discarded, ours is used instead
#   rows  8-15  torso and arms  -> the TOP sprite's lower half
#   rows 16-23  legs            -> the LEG sprite
#
# FOUR LEG POSES, ONE PER FILE, and this is the real gain. The cycle in
# KEYSTONE.bas already indexes four (hanim AND 8, AND 16) but only two were
# ever drawn -- HLEG3/HLEG4 were the first two MIRRORED, which is a decent
# cheat for a stride but reads as a limp next to four genuine poses. Now each
# beat is its own drawing and no mirroring is involved.
#
# TWO BODY POSES, from files 1 and 3 -- the extremes of the arm swing. The
# source picks the body on `hanim AND 8`, so it can only use two; taking the
# extremes rather than adjacent frames makes the swing read at that rate.
...
def _leg_split_rows(rows):
    ...
    ends = {(r, c) for r in range(8) for c in range(16)
            if on(r, c) and not any(on(r + 1, c + d) for d in (-1, 0, 1))}
    shoe = ends | {(r - 1, c) for (r, c) in ends}
...
# frame without an arm there gets a black bar hanging in mid-air beside him.
# ONE STRIPE ART PER FRAME, not one shared between them. It used to be shared,
# which forced every stripe off the rows the arms move on -- and with the arms
# now three rows deep and the bands every other row, that would have left the
# whole middle of him blank. A second drawing costs ONE MORE SPRITE PATTERN
# and NO extra sprite box (the stripe is already its own box), so the per-line
# budget in DESIGN.md 5 is untouched: it is 32 bytes of pattern table.
# THE FACE BOX IS PUSHED TO THE BOTTOM OF ITS SPRITE so that the box covers
# rows -7..8 instead of 3..18. The VDP counts BOXES per scanline, not pixels, so
# where that box reaches is what costs -- and reaching down to row 18 put a
# third Harry box on every torso row. Against Kelly's two that is five on a
# line, one over the limit, and the VDP drops the highest: his STRIPES. They
# vanished exactly when the two actors were level, which is most of the game.
#
# With the box lifted, the torso rows carry body + stripe only. Harry costs two
# there, a meeting is four, and nothing drops. What is left is rows 0-8, where
# the only stripe is the hat's two bands -- so if anything still overflows, he
# loses the band on his cap and keeps every stripe on his body.
HARRY_FACE = shift(band(HARRY_TOP, HFACEB), 7)          # drawn at y-7


def cut(a, b):
    """Remove b's pixels from a -- the same hole-punching the torso needs."""
    nl = chr(10)
    ra = a.strip(nl).split(nl)
    rb = b.strip(nl).split(nl)
    return nl + nl.join(
        "".join("." if y == "#" else x for x, y in zip(x_row, y_row))
        for x_row, y_row in zip(ra, rb)) + nl


# (HARRY_LEG1/2 are already the white half -- see leg_stripe in the note above
# the leg art. cut() stays because it is the general tool for punching one
# sprite out of another.)
...
# THE CROUCH IS CARVED THE SAME WAY THE RUN IS, and drawn at the same offsets.
#
# It used to be three 16x16 grids all drawn at ONE y, overlapping, with the
# helmet winning shared pixels -- which needed a reader of its own and cost
# THREE sprites on every scanline the figure touched, because the VDP counts
# sprite BOXES and not the ink in them.
#
# Staggered like the run (hat drawn at y-10, face at y-5, body at y+11) the
# three boxes span -10..5, -5..10 and 11..26: the face and the body never share
# a scanline, so no line carries more than two. With two obstacles that is four,
# which is exactly the VDP's limit -- the crouch stopped being the thing that
# pushed a line over it.
KELLY_DHAT = shift(band(KELLY_DTOP, HAT), 10)           # drawn at y-10
KELLY_DFACE = shift(band(KELLY_DTOP, FACE), 5)          # drawn at y-5
KELLY_DBODY = overlay(shift(band(KELLY_DTOP, TORSO), -11),
                      shift(KELLY_DLEG, 5))             # drawn at y+11
...
SPRITES = [
    ...
    # THE RADAR MARKER. Three by three in the corner of a 16x16 box, which is
...
""", WHITE, STORE_BG),
    ...
    # that row beside the score. White on dark blue made the spare Kops a
    # floating blue tile with a white blob on it rather than three helmets on
    # the same strip as the numbers.
    ("KOPIC", 0, """
...##...
..####..
.######.
...
""", LYELL, BLDG_BODY),
    ...
    # three of brick for a low block, two over six for a tall one.
    ("BLDGL", 0, """
........
........
........
...
""", LYELL, SKYGRAD[2][:2] + [BLDG_BODY] * 6),
    ...
    # may be a full storey, and a top row -- and the top row holds either this
    # or a full BLDGW. So the tallest building on the horizon is three whole
...
""", [WHITE] * 6 + [LYELL, LYELL], [CYAN] * 6 + [DYELL, DYELL]),
    ...
    # with the step along its bottom two pixels.
    #
    # It was left out on the reasoning that "a sill under a shut door would be
    # a ledge with nothing behind it" -- which sounds right and is wrong. A
    # real lift threshold is a plate set into the floor in front of the doors;
    # it belongs to the landing, not to the car, and you can see it whether the
    # car is there or not. Leaving it off made the doorway sit flush with the
    # wall on every floor the car was not on, so the elevator read as a painted
    # rectangle rather than as something you could walk into.
    #
    # Same two pixels and the same LYELL-over-DYELL as the floor bar's own top
    # edge (see ECARS), so the step is continuous across all four columns of
    # the frame in every door state.
    ("EDOORS", 0, """
........
........
........
...
""", [WHITE] * 6 + [LYELL, LYELL], [GRAY] * 6 + [DYELL, DYELL]),
    ...
    # THE HEADER ROW, HALF LINTEL AND HALF DOORWAY. The opening used to start
    # at band row 1, so it was 24 px in a 32 px band and Kelly -- 24 px on a
    # 2 px step -- stood with his hat in the lintel. Giving row 0's bottom half
    # to the door makes the opening 28 px and leaves 4 px of lintel above it,
    # which is enough to still read as a frame.
    #
...
""", GRAY, STORE_BG),
    ...
    # set into the shop rather than as brickwork closing it off, and it did so
...
""")
        ...
        # Three distinct eight-byte blocks, expanded across sixteen columns
        # by this generator. Store the 384-byte result in setup bank 2 so a
        # DEFINE COLOR replaces the game's fixed-area expansion loop.
        ...
        #
        # WHY IT HAS TO WORK THAT WAY: a colour byte covers one 8x1 scan line
        # of one CHARACTER, so a whole pixel row of the canvas can only be one
        # colour -- which is why the Kop and the crook used to get a row each
        # and stood one pixel high. Filling the band means both want all three
        # rows, and they can have them because they are almost never in the
        # same character: eight pixels of this canvas is half a screen of the
        # store. So the row's colour is set from whoever is standing in that
        # character. When they ARE in one, the later writer wins and they merge
        # -- which is the honest picture anyway, since at that range the Kop is
        # on top of him.
        #
        # Grey is what the 2600 has: measured off the reference video,
        ...
        for crow in range(3):
            for line in range(8):
                ...
                if row < SCAN_TOP or row >= SCAN_TOP + 16:
                    ...
                    fg = (GRAY, GRAY, GRAY, LYELL)[(row - SCAN_TOP) % 4]
                    bg = DGREEN
                scol.append((fg << 4) | bg)
        # Expand each character row sixteen times. DEFINE COLOR uploads the
        # complete 48-character canvas to all three TMS screen thirds.
        # Separate output: setup bank 2 has space; runtime bank 1 is tight.
        expanded = [v for crow in range(3) for _ in range(16)
                    for v in scol[crow * 8:crow * 8 + 8]]
        with open(os.path.join(here, "..", "src", "scancol.bas"), "w") as scan:
            scan.write("\t' GENERATED by assets/genart.py; setup-bank radar colours.\n")
            emit(scan, "scan_cols", expanded, "48 chars, eight colour bytes each")
