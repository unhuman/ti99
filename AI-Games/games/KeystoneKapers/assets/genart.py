...
""", LYELL, [CYAN] * 2 + [DRED] * 6),
    ...
    # THE TWO SHALLOW TOPS, which is what caps the skyline's height. The city
    # is drawn over three rows: a solid row of wall, a middle row that may be
    # a full storey, and a top row that may only ever be one of these. So the
    # tallest building on the horizon is two whole characters plus four
    # pixels -- half a character over the flat roofline -- and no more.
    #
    # A slice of the wall taken from its BOTTOM, so the windows land where
    # BLDGW's do and the courses line up cell to cell: rows 4-7 of the wall
    # for the 4px top, rows 6-7 for the 2px one (which is blank wall, and so
    # reads as a plain parapet).
    ("BLDGM", 0, """
........
........
........
