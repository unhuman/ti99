...
- **Status:** 🎮 Playable — M1–M3 emulator-verified in Classic99, M4 core in (title, rounds,
  challenge stages, jingles). Deferred: in-game music, maps 2–4, rocks, per-round flag sets,
  Coleco runtime pass (see `DESIGN.md` §17). TI cart is **banked** (`BANK ROM 128`, data in
  bank 1); Coleco builds flat. Building this flushed out three CVBasic TMS9900 codegen bugs
  (8-bit×big-constant → 0, dotted-constant folding truncates, CONST>255 truncates) — all
  documented in `DESIGN.md` §14 with workarounds in the source.
