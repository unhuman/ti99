...
- **ROM-resident tables:** per-game link tables and a per-room wall-colour byte are read on demand
  with RESTORE + skip-loops; room bitmaps are **row-dictionary compressed** (below). Space-savers
  that keep the build under the 24K ceiling (see the note below):
  - **Row-dictionary rooms:** the 29 distinct rooms use only **71 distinct 4-byte rows**, so those
    rows live once in `rowdd` (284 B, loaded into the `rowd()` RAM array at startup) and each room
    is just **24 one-byte indices** into it (696 B). `rbl` expands the 24 indices into `rm()` from
    the RAM dictionary on room entry (fast — no per-row RESTORE). 980 B total vs 2,784 B of raw
    bitmaps — **reclaimed ~1,800 bytes** (at the cost of 284 B RAM). Round-trip verified
    byte-identical to the raw rooms.
  - **Bitmap dedup:** 10 of the 39 rooms have wall bitmaps byte-identical to an earlier room, so
    they share one dictionary index-block (29 distinct rooms stored) — reclaimed 960 bytes. Those
    10 rooms also have no loader block of their own — their `ON rn GOTO` slot jumps to the
    canonical room's `rb` — reclaimed ~120 more bytes.
...
- Rooms are dispatched by a 39-slot `ON rn GOTO` ladder to per-room `RESTORE`s; each room's `DATA`
  is now **24 row-indices** (not 96 raw bytes), which `rbl` expands into `rm()` via the `rowd()`
  dictionary (loaded once at startup from `rowdd`). Links/colors are ROM `DATA` too; player links
  cached per room (`pn/pe/ps/pw`), bat looks links up on demand. Ten rooms whose wall bitmaps are
  byte-identical to an earlier room have **no index block of their own**: their `ON rn GOTO` slot
  jumps straight to the canonical room's `rb` (`rn 24→rb13`, `26/31/34→rb19`, `27/30→rb20`,
  `23/29→rb21`, `33→rb25`, `35→rb28`). Because colors/gates/objects come from the *separate*
  per-room tables (indexed by `rn`), shared-bitmap rooms still look and play differently — only the
  wall geometry is shared.
- **Editing a room:** rooms are indices into `rowdd`, so hand-editing hex no longer works directly.
  Take the raw `DATA BYTE` rooms from the last commit *before* the row-dictionary reclaim, edit them,
  then run `tools/gendict.py raw_rooms.bas` (parses raw rooms → dictionary + indices, round-trip-
  verifies, writes `newdata.txt`) and paste the result back in place of the `rowdd`+`rdN` blocks.
- **24K RAM ceiling (why the space-savers matter):** the CVBasic TI-99 backend copies a *fixed*
  ...
  1. **`linkticart` 24,336 B (the real limit):** it emits the program as three 8,112-byte loader
     ...
     `len(adventire.bin) − 16384` must be **≤ 24,336** (after the row-dictionary reclaim the
     current build is 22,699 — **1,637 B clear**, plenty of room again).
  ...
  original build overflowed by ~495 B; the reclaims — **row-dictionary rooms (~1,800 B)**, bitmap
  dedup (960 B), collapsed loader blocks for the shared rooms (~120 B), colour-byte (~63 B), `lnkb`
  trim (52 B) — put the build ~1.6 KB under the ceiling with all features in.
