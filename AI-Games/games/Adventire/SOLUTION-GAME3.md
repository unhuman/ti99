# Adventire — Game 3 (FULL KINGDOM) Walkthrough

The whole kingdom: three castles, three dragons, the bat, the magnet, two DARK zones with
fog-of-war lamplight, and the hidden dot with its secret room. You win by carrying the
**chalice** into the **gold castle hall**. One object at a time (touch to pick up / swap,
FIRE to drop). Dragons ignore walls and pursue you between rooms (~1.3s behind); slain
dragons stay dead all game, so the sword sweep comes first.

## The map (with each room's on-screen wall color)

```
                                [BLACK CASTLE (black)]---(gate)---[hall]--[DARK black maze:
                                        |                                  26 - 27 - 28
                                [blue maze top (dark blue)]                MAGNET in 27,
                                        |                                  CHALICE in 28,
                     [maze mid (RED DRAGON)]--[maze cross: WHITE KEY]      DOT in 28's
                                        |                                  sealed chamber]
[GOLD CASTLE (yellow; start/WIN)]  [maze entry]
              |                         |
[corridor W (dark green)]--[corridor mid (green): SWORD]--[corridor E (yellow):
                                                           YELLOW DRAGON; secret wall E]
                                                                |
                                              [DARK catacombs: 29 (GREEN DRAGON)
...
walls there are gray on black. The **bat** starts in the blue maze entry and steals
whatever it touches — chase it and touch your item to snatch it back.

## Step by step (each move = one screen)

Start: standing below the yellow gold castle.

1. **Sword.** DOWN 1 (green corridor): take the **sword**.
2. **Slay the yellow dragon.** RIGHT 1 (yellow corridor): it charges — let it hit the
   blade.
3. **Slay the green dragon.** DOWN 1 (into the DARK catacombs): it looms out of the black —
   hold the sword toward it.
...
- The purple/cyan/rose side rooms east of the catacombs (RIGHT from the third dark
  screen) are a scenic dead end in this game — nothing required lives there.
- The bat can relocate anything you left lying around, including the sword and keys. If
  something is missing, wander until you spot the bat's cargo, or drop the magnet in the
  room where an object hides inside a wall.
- Swallowed? FIRE returns to the title; the whole kingdom resets.
