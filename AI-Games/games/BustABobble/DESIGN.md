...
**32 KB and one bank** -- the combined cart's bank is 7,574 of 8,192.

### The combined cart

`--both` compiles `levels12.bas`: **ONE table of 80**, arcade at 1-30 and expert at
31-80, generated by `assets/genboth.py`. Not the two files side by side -- they each
define `pb_lay`, `pb_seq` and `pb_meta`, so including both would define every label
twice.

The engine needs no second set of pointers for it. A chosen set is a **base**
(0 or 30) added when indexing the table and a **count** (30 or 50) for the victory
test and the 838 clamp. What the HUD prints is still round 1..30 or 1..50, because
that is `lvl` -- the round *within* the set. The single carts set base 0 and their
own count once at boot and never touch them again.
