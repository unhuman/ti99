...
- **Lose a life:** the ship touches an asteroid or is hit by a UFO bullet. Start with 3 lives;
  game over at zero. Extra life every 10,000 points (no cap).
...
- **The ship pops fully across edges (no straddle/slide-in).** Its 32-px sprite box is kept
  ...
  **Y 16..176** (`#spy` 1024..11264). Reaching one limit jumps it to the opposite limit, fully
  visible. (Cost: the ship wraps ~16 px shy of the physical edge — half the sprite box.)
...
two-branch wrap. Render guard hides a rock within ~16 px of an edge (brief, acceptable for many
small objects, unlike the always-visible ship).
...
- **Spawn:** `#utimer` counts down — 600 frames (~10 s) to the first UFO, then 1200 (wave<4) /
  900 recurring. Wave <4 spawns large only; wave ≥4 is 40% small / 60% large.
- **Movement:** enters from a random edge at ±2 px/frame, wanders vertically, and **despawns when
  it reaches the far edge** (direction-aware test — `ux` is 8-bit, so the old `<0`/`>255` tests
  were dead code and the UFO never left).
- **Fire:** large UFO fires a random direction every 90 frames; small UFO aims at the ship
  (closest of 16 angles via `ufo_aim`) every 60 frames. One UFO bullet at a time; it's cleared
  when the UFO leaves or dies (no stranded dot).
- **Death:** when shot, the UFO plays the shared explosion in place (`uexp`), can't be re-hit
  while exploding, and scores once.
- **Sound:** channel 2 warbles while a UFO is active, pitched by size.
...
Channels 0–2 tone, 3 noise; each has an 8-bit `sfx` timer that silences the channel at 0.
Heartbeat (ch 0) alternates two low tones, tempo `hbeat_rate = 120 - ast_count*4` (min 30) so it
quickens as the field thins. Thrust = ch-3 noise while held; fire/explosions/UFO drone/ship-death
each drive their channel with a timer.
...
`upd_ast` + `render`). FIRE starts the game.
...
- [x] Collisions register from any direction (16-bit ABS); ship dies on rock/UFO-bullet contact
- [x] UFO appears (~10 s), moves, despawns at the far edge, fires (random/aimed), explodes when shot
...
- [x] Ship always fully visible and pops across edges; N/S/E/W frames symmetric
- [x] Wave clear gives a free-flight gap (no re-center) before the next wave
- [x] Title centered with drifting rocks, aligned controls, credit, and top HUD scores
