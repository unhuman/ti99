<#
.SYNOPSIS
  Screenshot (and optionally drive) the running iNES window.

.DESCRIPTION
  Two traps make this harder than it looks, and both were paid for already:

  * iNES's Process.MainWindowHandle is a HIDDEN 0x0 window. Capturing or
    activating that gets you nothing, and activating it leaves the real window
    INACTIVE -- which matters because iNES's `pauseinactive` option then
  ...
  * SPACE is joystick FIRE, but it only reaches the emulator while the running
...
.EXAMPLE
  powershell -File tools\shoot-ines.ps1 -Out shot.png
  powershell -File tools\shoot-ines.ps1 -Keys "0x20" -HoldMs 200 -Out after.png
#>
[CmdletBinding()]
param(
    [string]$Out = "iNES.png",
...
Add-Type -TypeDefinition 'using System.Runtime.InteropServices; public class InesDpi { [DllImport("user32.dll")] public static extern bool SetProcessDPIAware(); }'
[void][InesDpi]::SetProcessDPIAware()
...
$proc = Get-Process iNES -ErrorAction SilentlyContinue | Select-Object -First 1
if (-not $proc) { Write-Error "iNES is not running"; exit 1 }
...
if ($script:best -eq [IntPtr]::Zero) { Write-Error "no visible iNES window"; exit 1 }
$h = $script:best
...
# thing -- which reads as "iNES ignored the keys" or "the fix did not
...
if ([W99]::GetForegroundWindow() -ne $h) {
    Write-Warning ("iNES is NOT the foreground window -- keys will go " +
