...
| Astiroids | `games/Asteroids/` | `ASTIROIDS` | 🎮 Playable — first version | Classic Asteroids. CVBasic (`--ti994a`), cartridge ROM. 2× sprite magnification, 32-slot sprite pool (24 asteroid slots), 16-step ship rotation, inertial thrust + inertia-carrying bullets, UFO (large/small) with aiming and explosions, waves with free-flight gap, heartbeat sound, title screen, persistent session high score. |
