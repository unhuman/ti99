...
## 1. Toolchain & Target -> **skill `xb256-reference`**

The XB256/XB-compiler toolchain (hardware model, Classic99 with `JUWEL7` as DSK1, the `-X` XB-loader
default, the TI no-period filename rule, and the 6-file `NAME`/`-M`/`-S`/`-O`/`-E`/`-X` pipeline),
the **Memory Budget** (former §5) and the **Reference Assets** list (former §7) now live in
`.claude/skills/xb256-reference/SKILL.md`, alongside the XB256 API map §3 already moved there.

**Invoke that skill when working on an XB256 game** (`games/dotmuncher`, `games/mspacman`). The
binding parts stayed in this file: the compiler landmines (§2), the performance budget (§5A), the
checklist (§6), the arcade conventions (§7A) and the per-game lifecycle (§8).
